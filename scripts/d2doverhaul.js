...
function windowProc(hwnd, msg, wp, lp) {
    if(msg == WM_CREATE) {
        ...
        //brush.SetTransform(d2d.Matrix3x2.Translation(0, 256)); //only used by bitmap brushes or gradient brushes
        i = 0;
        SetTimer(hwnd, 0, 16);
        ...
        //bmpBrush.SetTransform(d2d.Matrix3x2.Translation(i, i));
        //watch out bruh Matrix3x2 is global
        gradient.SetTransform(Matrix3x2.SetProduct(Matrix3x2.Translation(100+Math.sin(i/100)*100, 0), Matrix3x2.Rotation(i, 128+(100+Math.sin(i/100)*100), 128)));

        d2d.SaveDrawingState();

        //d2d.SetTransform(d2d.Matrix3x2.Rotation(i, 256, 256));
        //const transPtr = d2d.Matrix3x2.FromMatrix(d2d.Matrix3x2.Translation(256, 256)); //yeah this system is too weird i gotta change it
        //const rotPtr = d2d.Matrix3x2.FromMatrix(d2d.Matrix3x2.Rotation(i, 256, 256));
        //d2d.SetTransform(d2d.Matrix3x2.SetProduct(transPtr, rotPtr));
        d2d.SetTransform(Matrix3x2.SetProduct(Matrix3x2.Translation(256, 256), Matrix3x2.Rotation(i, 256, 256))); //oh yeah that's better

        //gradient.SetTransform(d2d.Matrix3x2.SetProduct(d2d.Matrix3x2.Translation(256, 256), d2d.Matrix3x2.Rotation(i, 256, 256)));
        ...
        //d2d.Matrix3x2.DeleteMatrix(transPtr); //oops i was wondering why the memory was still climbing but i forgot that FromMatrix returns a pointer (unfortunately a really weird system)
        //d2d.Matrix3x2.DeleteMatrix(rotPtr);

        ////if(GetKey(VK_LBUTTON)) {
        ////    d2d.SetTransform(d2d.Matrix3x2.Rotation(i, 256, 256));
        ////}
        ////d2d.SaveDrawingState();
        ////d2d.Matrix3x2.Translation(256, 256);
        //d2d.FillRectangle(-25, -25, 25, 25, brush);
        //let transPtr = d2d.Matrix3x2.FromMatrix(d2d.Matrix3x2.Translation(256, 256));
        //let rotPtr = d2d.Matrix3x2.FromMatrix(d2d.Matrix3x2.Rotation(i, 256, 256));
        //d2d.SetTransform(d2d.Matrix3x2.SetProduct(transPtr, rotPtr));
        ////d2d.SetTransform(d2d.Matrix3x2.Translation(256,256));
        ...
        ////d2d.SetTransform(d2d.Matrix3x2.Identity());
        //brush = d2d.CreateSolidColorBrush(1.0, 1.0, 0.0);
