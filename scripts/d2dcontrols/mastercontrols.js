...
class TextButtonControl extends StaticControl {
    constructor(width, height, value, textcolor, buttoncolor = [0, 0, 0], callback, parent) {
        super(width, height, value, textcolor);
...
class CheckboxControl extends GenericControl {
    ...
    constructor(width = 16, height = 16, value = false, callback, parent) {
        super();
        this.width = width;
        this.height = height;
        this.value = value;
...
class EditDropdownControl extends GenericControl {
    ...
    constructor(width, height, options, value, submitCallback, delayedOptionsCallback, parent) {
        super();
        this.width = width;
        this.height = height;
        this.options = options;
...
class EditControl extends GenericControl {
    ...
    constructor(width, height, data, value, endInput, parent) {
        super();
...
class HorizontalListBox extends GenericControl {
    children = [];
    ...
    redraw(x, y) {
        for(const child of this.children) {
            child.redraw(x+child.x, y+child.y);
        }
    }
...
class D2DWindow {
    ...
    init(hwnd) {
        const initobj = {};
        initobj.d2d = createCanvas("d2d", ID2D1RenderTarget, hwnd);
    ...
    windowProc(hwnd, msg, wp, lp) {
        this.realwindowproc(hwnd, msg, wp, lp);
