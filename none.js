...
while (!GetKey(VK_ESCAPE)) {
    ...
    d2d.FillRectangle(720, 540, 720+50,540+50, brush);
    d2d.DrawGradientText("lol" + Math.random(), font, 200, 800, 1920, 1080, gradientBrush);
    d2d.DrawBitmap(bitmap, mouse.x-(bitmapSize.width/2), mouse.y-(bitmapSize.height/2), mouse.x+(bitmapSize.width/2), mouse.y+(bitmapSize.height/2), 0.1);

    d2d.DrawRoundedRectangle(800, 800, 900, 900, (mouse.x / 1920) * 20, (mouse.y / 1080) * 20, brush);
    d2d.FillRoundedRectangle(810, 810, 890, 890, (mouse.x / 1920) * 20, (mouse.y / 1080) * 20, brush);

    d2d.FillRectangle(1920 - 256, 1000 - 256, 1920, 1000, gradientBrush);

    d2d.FillGradientEllipse(mouse.x, mouse.y, Math.sin(i / 10) * 50, Math.sin(i / 10) * 50, gradientBrush, j / 50);//i*50);
    d2d.DrawGradientText("sped: "+Math.round(j/5000), font, mouse.x-75,mouse.y+25, 1920, 1080, radialBrush);
    ...
    d2d.DrawGradientRectangle(mouse.x, mouse.y, mouse.x + 256, mouse.y + 256, gradientBrush, j / 50);
    d2d.FillGradientRectangle(mouse.x+25, mouse.y+25, mouse.x + 231, mouse.y + 231, gradientBrush, j/50);
    ...
    d2d.FillGradientRoundedRectangle(Math.abs(Math.sin(i / 100))*1920-90, 710, Math.abs(Math.sin(i / 100))*1920+90, 890, 25, 25, gradientBrush, i*5);//(mouse.x / 1920)*45);
    d2d.DrawGradientRoundedRectangle(Math.abs(Math.sin(i / 100))*1920-100, 700, Math.abs(Math.sin(i / 100))*1920+100, 900, 25, 25, gradientBrush, i*5 , 5);//(mouse.x / 1920)*45);
